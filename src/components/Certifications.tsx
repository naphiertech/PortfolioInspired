  ...
  return (
    <section className="w-full space-y-6 select-none mb-16" aria-label="Certifications">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-4.5">
        {featuredCerts.map((cert) => {
          ...
          return (
            <div
              key={cert.name}
              className="relative p-4 sm:p-4.5 rounded-xl bg-surface/25 border border-border-hairline hover:bg-surface/60 transition-all duration-200 group flex flex-col justify-between overflow-hidden shadow-2xs"
            >
              {/* Subtle decorative curve in dark mode */}
              <svg
                className="absolute right-0 top-0 w-28 h-28 pointer-events-none opacity-5 dark:opacity-10 stroke-muted-foreground/40 transition-opacity group-hover:opacity-20"
                viewBox="0 0 100 100"
                fill="none"
              >
              ...
              <div className="pt-3 mt-3 border-t border-border-hairline flex items-center justify-between gap-2 relative z-10">
                {cert.code ? (
      ...
      <div className="pt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-muted-foreground">
        <div className="border-l-2 border-border-hairline pl-3 italic text-muted-foreground/80">
